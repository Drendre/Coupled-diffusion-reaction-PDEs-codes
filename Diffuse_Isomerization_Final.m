%% Diffusion cascade code  The Time-Decaying Isomerization
clear; 
%% Parameters
S=7; %number of different time step sizes 
D=1; B=0.26; k1=3; alpha=0.5; gamma=B+alpha;%% coefficients
c1=1; c2=0; c3=0; c4=0; %% coefficients
ti=0.04; TIME=0.24; tf=ti+TIME;  %% initial time, Length of time interval, final time 
x0=-2; Lx=4; xf=Lx+x0; %% Space domain
Nx=800;Nx=Nx+1;Dx=(xf-x0)/(Nx-1); %% Space discretization abd number of discreation 
...
q=(sqrt(4*k1^2-2*alpha*gamma+gamma^2+alpha^2))/2;
f1=gamma-alpha+2*q; f2=gamma-alpha-2*q;
%%%%%%% Initial cond.
for i=1:Nx
kk=xaxis(i)^2/ti; 
u0(i)=xaxis(i)/ti^(1/2)/(ti^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)); %+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4)); %% function one 
w0(i)=xaxis(i)/ti^(1/2)/(ti^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2); %% second function
%%% Final analitical solution
kk=xaxis(i)^2/tf;
uex(i)=xaxis(i)/tf^(1/2)/(tf^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
wex(i)=xaxis(i)/tf^(1/2)/(tf^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
end
...
%%%%%%%%%%%%%%
w=zeros(Nx,1);w=zeros(Nx,1);u=zeros(Nx,1);a0=zeros(Nx,1);a01=zeros(Nx,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ih=1:S  %% Big loop for the timestep, ih is the Serial Number of run
    ih
...
tic
%%% Boundary conditions  for CCL,CLQ and CpC method left and right
for tind=1:T %%tind is the index of the time, always integer
    t=ti+tind*h;  %t is the physical time in the system
    kk=x0^2/t;
bound1u(tind)=Inorm*x0/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
bound1w(tind)=Inorm*x0/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
   kk=xf^2/t; 
boundNu(tind)=Inorm*xf/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
boundNw(tind)=Inorm*xf/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
t=ti+(tind-1/2)*h;  kk=x0^2/t; 
bound1u12(tind)=Inorm*x0/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
bound1w12(tind)=Inorm*x0/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
kk=xf^2/t; 
boundNu12(tind)=Inorm*xf/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
boundNw12(tind)=Inorm*xf/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
t=ti+(tind-2/3)*h;  kk=x0^2/t; 
bound1u13(tind)=Inorm*x0/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
bound1w13(tind)=Inorm*x0/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
kk=xf^2/t; 
boundNu13(tind)=Inorm*xf/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
boundNw13(tind)=Inorm*xf/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
t=ti+(tind-1/3)*h;  kk=x0^2/t; 
bound1u23(tind)=Inorm*x0/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
bound1w23(tind)=Inorm*x0/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
kk=xf^2/t; 
boundNu23(tind)=Inorm*xf/t^(1/2)/(t^alpha)*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4));%+c2*hypergeom(p+q+k1,3/2,kk/4)+c3*kummerU(p-q+k1,3/2,kk/4)+c4*kummerU(p+q+k1,3/2,kk/4));
boundNw23(tind)=Inorm*xf/t^(1/2)/(t^gamma)/2/k1*exp(-kk/4)*(c1*hypergeom(p-q+k1,3/2,kk/4)*f1);%+c2*hypergeom(p+q+k1,3/2,kk/4)*f2+c3*kummerU(p-q+k1,3/2,kk/4)*f1+c4*kummerU(p+q+k1,3/2,kk/4)*f2);
end
toc
%% The numerical Method start here
%% FTCS method 
u=u0; w=w0;
%% Big loop for time
for tind=1:T-1 %%tind is the index of the time, always integer
    t=ti+(tind-1/2)*h;
  for i=2:Nx-1
        rt=h*k1*(u(i)-w(i)*t^B)/t;
        rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
        uTEMP(i)=u(i)+r*(u(i-1)+u(i+1)-2*u(i))-rt;%%% diffusion equation
        wTEMP(i)=w(i)+r*(w(i-1)+w(i+1)-2*w(i))+rt1; %%% diffusion equation
  end
...
Diff=abs(uex-u); % error
if(Diff<1000) MaxD(ih,1)=max(Diff); end %% Max error
Diff=abs(wex-w);
if(Diff<1000) MaxD(ih,2)=max(Diff); end %% Max error
UEE=u;
%% Adam-Bashforth method2 AB2
 u=u0;u2=u; % these will be the latest values
 w=w0;w2=w; % these will be the old values
 tic
 t=ti+1/2*h;
for i=2:Nx-1 %%% zero-th stage, the latest/new values are updated
    rt=h*k1*(u(i)-w(i)*t^B)/t;
    rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
    u(i)=u(i)*(1-2*r)+(u(i-1)+u(i+1))*r-rt; 
    w(i)=w(i)*(1-2*r)+(w(i-1)+w(i+1))*r+rt1;
end
...
%% Big loop for time
 for tind=2:T-1 %%tind is the index of the time, always integer
    t=ti+(tind-1/2)*h;
f10=zeros(Nx,1);f20=zeros(Nx,1);
f11=zeros(Nx,1);f21=zeros(Nx,1);
for i=2:Nx-1   %%% first stage 
   rt=h*k1*(u(i)-w(i)*t^B)/t;
   rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
   f10(i)=u(i)*(-2*r)+(u(i-1)+u(i+1))*r-rt; %% using the new values
   f11(i)=w(i)*(-2*r)+(w(i-1)+w(i+1))*r+rt1;%% using the new values
     rt=h*k1*(u2(i)-w2(i)*t^B)/t;
     rt1=h*k1*(u2(i)*t^(-B)-w2(i))/t;
   f20(i)=u2(i)*(-2*r)+(u2(i-1)+u2(i+1))*r-rt;  %% using the old values
   f21(i)=w2(i)*(-2*r)+(w2(i-1)+w2(i+1))*r+rt1; %% using the old values
end
...
Diff=abs(uex-u); %%% error
if(Diff<1000) MaxD(ih,3)=max(Diff); end %% Max error
Diff=abs(wex-w);
if(Diff<1000) MaxD(ih,4)=max(Diff); end %% Max error
%% ADE method 
u=u0;w=w0;tic
% t=ti+1/2*h;
for tind=1:T-1 %%tind is the index of the time, always integer
...
rt=h*k1*(u(i)-w(i)*t^B)/t;
rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
a0(i)=(uTEMP(i-1)+uTEMP(i+1)-uTEMP(i))*r-rt;
a01(i)=(wTEMP(i-1)+wTEMP(i+1)-wTEMP(i))*r+rt1;
 uTEMP(i)=(uTEMP(i)+a0(i))/(1+r); 
 wTEMP(i)=(wTEMP(i)+a01(i))/(1+r);
end
 uTEMP1(Nx)=boundNu(tind); wTEMP1(Nx)=boundNw(tind);
for i=2:Nx-1    %%% from left to left 
j=Nx-i+1 ;
rt=h*k1*(uTEMP(j)-wTEMP(j)*t^B)/t;
 rt1=h*k1*(uTEMP(j)*t^(-B)-wTEMP(j))/t;
au(j)=(uTEMP1(j-1)+uTEMP1(j+1)-uTEMP1(j))*r-rt;
  aw(j)=(wTEMP1(j-1)+wTEMP1(j+1)-wTEMP1(j))*r+rt1;
...
MaxD(ih,6)=err_w;
AbsDU_UADE=max(abs(u-uex));MaxD(ih,5)=max(AbsDU_UADE);%% Max error
AbsDU_UADE1=max(abs(w-wex));MaxD(ih,6)=max(AbsDU_UADE1);%% Max error
...
tic
  for i=2:Nx-1  %%% zero-th stage, UPFD formula
      rt=h*k1*(u(i)-w(i)*t^B)/t;
      rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
      u(i)=(u(i)+r*(u(i-1)+u(i+1))-rt)/(1+2*r); 
      w(i)=(w(i)+r*(w(i-1)+w(i+1))+rt1)/(1+2*r);
  end
    u(1)=bound1u(1); u(Nx)=boundNu(1);  
    w(1)=bound1w(1); w(Nx)=boundNw(1);
for tind=2:T-1
    for i=2:Nx-1  %%% second stage 
       ...
       rt=h*k1*(u(i)-w(i)*t^B)/t;
       rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
       uTEMP(i)=((1-2*r)*uold(i)+2*r*(u(i-1)+u(i+1))-2*rt)/(1+2*r);
       wTEMP(i)=((1-2*r)*wold(i)+2*r*(w(i-1)+w(i+1))+2*rt1)/(1+2*r);
     end
   uTEMP(1)=bound1u(tind); uTEMP(Nx)=boundNu(tind);  
    wTEMP(1)=bound1w(tind); wTEMP(Nx)=boundNw(tind);
   uold=u;wold=w;
...
AbsDU_Uuodd1=max(abs(w-wex));MaxD(ih,10)=max(AbsDU_Uuodd1);%% Max error
...
for i=2:Nx-1   %%% first stage, predictor with half time step   
  rt=h*k1*(u(i)-w(i)*t^B)/(2*r*t);
rt1=h*k1*(u(i)*t^(-B)-w(i))/(2*r*t);
    a0=((u(i-1)+u(i+1))/2 -rt);
    a01=((w(i-1)+w(i+1))/2 +rt1);
...
for i=2:Nx-1 %%% second stage, corrector with full time step 
 rt=h*k1*(uTEMP(i)-wTEMP(i)*t^B)/(2*r*t);
rt1=h*k1*(uTEMP(i)*t^(-B)-wTEMP(i))/(2*r*t);
    a0(i)=((uTEMP(i+1)+uTEMP(i-1))/2-rt);
a01(i)=((wTEMP(i+1)+wTEMP(i-1))/2+rt1);
...
AbsDU_UCpC=max(abs(u-uex));MaxD(ih,11)=max(AbsDU_UCpC);%% Max error
AbsDU_UCpC1=max(abs(w-wex));MaxD(ih,12)=max(AbsDU_UCpC1);%% Max error
...
for i=2:Nx-1 %%% first satge 1/3 length
 rt=h*k1*(u(i)-w(i)*t^B)/(2*r*t);
rt1=h*k1*(u(i)*t^(-B)-w(i))/(2*r*t);
a0(i)=((u(i-1)+u(i+1))/2-rt);
a01(i)=((w(i-1)+w(i+1))/2+rt1);
...
rt=h*k1*(uTEMP(i)-wTEMP(i)*t^B)/(2*r*t);
rt1=h*k1*(uTEMP(i)*t^(-B)-wTEMP(i))/(2*r*t);
q1=((uTEMP(i-1)+uTEMP(i+1))/2-rt);
q2=((wTEMP(i-1)+wTEMP(i+1))/2+rt1);
...
rt=h*k1*(uTEMP1(i)-wTEMP1(i)*t^B)/(2*r*t);
rt1=h*k1*(uTEMP1(i)*t^(-B)-wTEMP1(i))/(2*r*t);
q1=((uTEMP1(i-1)+uTEMP1(i+1))/2-rt);
q2=((wTEMP1(i-1)+wTEMP1(i+1))/2+rt1);
...
AbsDU_UCCL=max(abs(u-uex));MaxD(ih,13)=max(AbsDU_UCCL);%% Max error
AbsDU_UCCL1=max(abs(w-wex));MaxD(ih,14)=max(AbsDU_UCCL1);%% Max error

%% Quadratic CLQ method 
...
rt=h*k1*(u(i)-w(i)*t^B)/(2*r*t);
      rt1=h*k1*(u(i)*t^(-B)-w(i))/(2*r*t);
        au0(i)=((u(i-1)+u(i+1))/2 -rt);
         aw0(i)=((w(i-1)+w(i+1))/2 +rt1);
         uTEMP(i)=u(i)*eed+ au0(i)*e1ed;
         wTEMP(i)=w(i)*eed+ aw0(i)*e1ed;
    end
    uTEMP(1)=bound1u(tind); uTEMP(Nx)=boundNu(tind);
    wTEMP(1)=bound1w(tind); wTEMP(Nx)=boundNw(tind);
   for i=2:Nx-1  %%%Second stage, CNe 
rt=h*k1*(uTEMP(i)-wTEMP(i)*t^B)/(2*r*t);
      rt1=h*k1*(uTEMP(i)*t^(-B)-wTEMP(i))/(2*r*t);
       aunew=((uTEMP(i-1)+uTEMP(i+1))/2 -rt);
      awnew=((wTEMP(i-1)+wTEMP(i+1))/2 +rt1);
   ...
   uTEMP2(1)=uTEMP(1);uTEMP2(Nx)=uTEMP(Nx);   wTEMP2(1)=wTEMP(1);wTEMP2(Nx)=wTEMP(Nx);
    for ii=1:2 %%% automatic repetition of the third stage as the fourth stage
     for i=2:Nx-1   %%% third stage, Quadratic neighbour
rt=h*k1*(uTEMP2(i)-wTEMP2(i)*t^B)/(2*r*t);
      rt1=h*k1*(uTEMP2(i)*t^(-B)-wTEMP2(i))/(2*r*t);
         aunew=((uTEMP2(i-1)+uTEMP2(i+1))/2 -rt);
         awnew=((wTEMP2(i-1)+wTEMP2(i+1))/2 +rt1);
 rt=h*k1*(uTEMP2(i)-wTEMP2(i)*t^B)/(2*r*tq);
      rt1=h*k1*(uTEMP2(i)*t^(-B)-wTEMP2(i))/(2*r*tq);
         aumid=((umid(i-1)+umid(i+1))/2 -rt);
         awmid=((wmid(i-1)+wmid(i+1))/2 +rt1);
...
L2D(ih,16)=err_w;
 ...
 AbsDU_UQ=max(abs(u-uex));MaxD(ih,15)=max(AbsDU_UQ);%% Max error
AbsDU_UQ1=max(abs(w-wex));MaxD(ih,16)=max(AbsDU_UQ1);%% Max error
...
rt=h/2*k1*(u(i)-w(i)*t^B)/t;
      rt1=h/2*k1*(u(i)*t^(-B)-w(i))/t;
    a0(i)=(u(i-1)+u(i+1))*r/2-rt;
a01(i)=(w(i-1)+w(i+1))*r/2+rt1;
...
for i=2:Nx-1   %%% second stage, full time step  
  rt=h*k1*(uTEMP(i)-wTEMP(i)*t^B)/t;
      rt1=h*k1*(uTEMP(i)*t^(-B)-wTEMP(i))/t;
    au(i)=(uTEMP(i-1)+uTEMP(i+1))*r-rt;
  aw(i)=(wTEMP(i-1)+wTEMP(i+1))*r+rt1;
...
AbsDU_UPI=max(abs(u-uex));MaxD(ih,17)=max(AbsDU_UPI);%% Max error
AbsDU_UPI1=max(abs(w-wex));MaxD(ih,18)=max(AbsDU_UPI1);%% Max error

%% Leapfrog-Hopschot method LH
u=u0; w=w0;  
tic
t=ti+h/4;
 for i=2:2:Nx-1  %%% zero-th stage for even nodes
        rt=h/2*k1*(u(i)-w(i)*t^B)/t;
      rt1=h/2*k1*(u(i)*t^(-B)-w(i))/t;
      u(i)=(u(i)+r/2*(u(i-1)+u(i+1))-rt)/(1+r); 
      w(i)=(w(i)+r/2*(w(i-1)+w(i+1))+rt1)/(1+r);
 end
%% Big loop for time 
for tind=1:T-1 %%tind is the index of the time, always integer
  ...
  for i=3:2:Nx-1  %%%  odd stage  
        rt=h*k1*(u(i)-w(i)*t^B)/t;
      rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
      u(i)=(r*(u(i-1)+u(i+1))+(1-r)*u(i)-rt)/(1+r);
     w(i)=(r*(w(i-1)+w(i+1))+(1-r)*w(i)+rt1)/(1+r);
  end
  t=t+h/2;uTEMP=u;
  for i=2:2:Nx-1  %%% even stage
    if (taxis(tind+1)<tf-0.8*h) %% if we are far from the end, full time step
      ...
      rt1=h*k1*(u(i)*t^(-B)-w(i))/t;
        ...
        rt=h/2*k1*(u(i)-w(i)*t^B)/t;
      rt1=h/2*k1*(u(i)*t^(-B)-w(i))/t;
         u(i)=(r/2*(u(i-1)+u(i+1))+(1-r/2)*u(i)-rt)/(1+r/2);
        w(i)=(r/2*(w(i-1)+w(i+1))+(1-r/2)*w(i)+rt1)/(1+r/2);
            end
    end
end
toc
ulh=u;wlh=w; % initialization
...
if(AbsDU_ULH<1000) MaxD(ih,19)=max(AbsDU_ULH); end %% Max error
AbsDU_ULH1=max(abs(w-wex));
if(AbsDU_ULH1<1000) MaxD(ih,20)=max(AbsDU_ULH1); end %% Max error
end %%% end of big loop for the set of time step sizes

%%Aggregated errors
for ie=1:20   
Sum_Log_MaxD(ie)=double(sum(log10(MaxD(:,ie))));
ARE(ie)=double((Sum_Log_MaxD(ie))); 
end
...
%%% figure 2:
 figure('Name', 'Max Errors as a fuction of time step h');   
	...
	hold on;
    plot(Axhstep(:),MaxD(:,3), '--h',  'Color', darkgreen, 'LineWidth', 4.5,'MarkerSize',11.5); %% Heun Ave
	hold on;
   ...
   plot(Axhstep(:),MaxD(:,15), '-o',  'Color', darkred, 'LineWidth', 4,'MarkerSize',8.5); %% Heun Ave
	...
	hold on;
   plot(Axhstep(:),MaxD(:,4), '--^',  'Color',  darkgreen2, 'LineWidth', 4.2,'MarkerSize',11); %% Heun Ave
	hold on;
...
%%%% Figure 3 (L2 Errors)
 figure('Name', 'L2D Errors as a fuction of time step h');   
	...
	hold on;
    plot(Axhstep(:),L2D(:,3), '--h',  'Color', darkgreen, 'LineWidth', 4.5,'MarkerSize',11.5); %% Heun Ave
	hold on;
   ...
   plot(Axhstep(:),L2D(:,15), '-o',  'Color', darkred, 'LineWidth', 4,'MarkerSize',8.5); %% Heun Ave
	...
	hold on;
   plot(Axhstep(:),L2D(:,4), '--^',  'Color',  darkgreen2, 'LineWidth', 4.2,'MarkerSize',11); %% Heun Ave
	hold on;
